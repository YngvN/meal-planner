...
export default async (req: Request) => {
  ...
  const tool: Anthropic.Tool = {
    ...
    input_schema: {
      ...
      properties: {
          ...
          items: {
            ...
            properties: {
              ...
              alternatives: {
                type: 'array',
                items: { type: 'string' },
                description: 'Other ingredient names that can substitute this one (e.g. ["olivenolje"] for "smør eller olivenolje"). Only the PRIMARY name goes in "name"; alternatives go here.',
              },
            },
        ...
        dietaryTags: {
          type: 'array',
          items: { type: 'string', enum: ['vegetarian', 'vegan', 'gluten-free', 'dairy-free', 'nut-free'] },
        },
        cuisineTypes: {
          type: 'array',
          items: { type: 'string' },
          description: 'Cuisine styles clearly identifiable from the recipe (e.g. "Italian", "Thai").',
        },
        equipment: {
          type: 'array',
          items: { type: 'string' },
          description: 'Kitchen equipment clearly required by the recipe (e.g. "blender", "stand mixer").',
        },
        skillLevel: {
          type: 'string',
          enum: ['beginner', 'intermediate', 'advanced'],
          description: 'Inferred difficulty based on techniques and steps.',
        },
        tags: {
          type: 'array',
          items: { type: 'string' },
          description: 'Short freeform descriptors (e.g. "quick", "one-pot", "spicy").',
        },
      },
  ...
  const textBlock: Anthropic.TextBlockParam = {
    ...
    text: [
      ...
      '- If an ingredient lists alternatives (e.g. "smør eller olivenolje"), put the first option in "name" and the rest in "alternatives"',
      '- Add mealTags, dietaryTags, cuisineTypes, equipment, skillLevel and tags only when you are confident',
      '',
